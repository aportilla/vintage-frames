...
| `emit`, `prefersReducedMotion`, `runSelectionBlink`, `runOutlineTravel`, `BLINK_INTERVAL_MS`, `BLINK_FLIPS`, `WALK_STEP_PX`, `WALK_STEP_MS`, `WALK_BEAT_MS`, `PRESS_HOLD_MS`, `RENAME_DELAY_MS` | The `bubbles`+`composed` event convention; the ~250ms selection blink; the walk — the cadence an icon's `dragTo` moves its outline at (system px per step, ms per step) and the beat between a field's landings; the tap-vs-hold threshold; the rename delay |
