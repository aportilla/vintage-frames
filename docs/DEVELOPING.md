...
Density comes two ways. By default the page builder sets it with Playwright's `deviceScaleFactor`, which is emulation: the page reports the density, but Chromium styles and lays it out as if at 1×, so a fractional border width floors to a whole CSS px and layout quantizes to 1/64 CSS px. A display does neither. `build(markup, { dpr, real: true })` renders at display density instead — a Chromium launched with `--force-device-scale-factor` and no viewport override (`browserAt`) — and is what checks about where a border puts content, or about the device grid, use: `verify:grid` and the display-density group in `verify:scrollbars`.
