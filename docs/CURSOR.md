...
The kit's chrome sets the classic cursors with ordinary CSS — the arrow on controls, the I-beam in an editable well — so a page with no setup gets System 7's pointer behavior at native crispness.
...
Stable Safari's top layer does not blend against the page, and no feature query can detect this, so on Apple engines each XOR cursor draws its `staticSrc` variant instead — same box, same hotspot, ink under a one-pixel white halo. Give your own `invert` art a `staticSrc` too.

Each kind accepts custom art: a `VfCursorArt` is frame URLs at one image px per system px, the size, the hotspot, and whether it draws with the XOR pen.
