...
| `requestGridSnap`, `GridSnapController` | Device-pixel-grid snapping is always on; `requestGridSnap()` re-checks after a move that resized nothing. Add snapping to your own component with one controller line plus the `vf-snap` class on its painted root |
...
| `vfBase`, `vfDisplay`, `vfDisplayDecls`, `vfBodyDecls`, `vfStaticText`, `vfPanel`, `vfChromeFrame`, `vfModalFrame`, `vfRule`, `vfTitleBar`, `vfWindowWidgets`, `vfHardShadowDecls`, `vfStripes`, `vfDots`, `vfFocus`, `vfFocusRing`, `vfFocusUnderline`, `vfToggle`, `vfField`, `vfScrollRail` | The 1-bit CSS recipes — compose into `static styles` |
| `RULE_EDGES`, `parseRule`, `ruleClasses` | The `rule` attribute grammar behind `vfRule`: the four edge names, the parser (edge names separated by whitespace, any order; anything else refuses the value), and the class list a resolved rule puts on a box — `vf-rule-top vf-rule-bottom` |
...
| `PATTERNS`, `PATTERN_NAMES`, `PATTERN_SIZE`, `parsePattern`, `patternHex`, `patternMotif`, `patternRects`, `PatternFillController`, `vfPatternFill` | The 38 standard MacPaint patterns as 8-byte QuickDraw patterns, the `pattern` attribute grammar (a name or sixteen hex digits), the adapters onto the tile machinery, and the fill that paints a pattern as a custom element's own box background — exact at every density and zoom ([PATTERNS.md](./PATTERNS.md)) |
...
| `emit`, `prefersReducedMotion`, `runSelectionBlink`, `runOutlineTravel`, `BLINK_INTERVAL_MS`, `BLINK_FLIPS`, `WALK_STEP_PX`, `WALK_STEP_MS`, `WALK_BEAT_MS`, `PRESS_HOLD_MS`, `RENAME_DELAY_MS`, `TAP_PAIR_MS`, `TAP_PAIR_SLOP_PX`, `DRAG_SLOP_PX`, `DRAG_SLOP_COARSE_PX`, `dragSlop` | The `bubbles`+`composed` event convention; the ~250ms selection blink; the walk — the cadence an icon's `dragTo` moves its outline at (system px per step, ms per step) and the beat between a field's landings; the tap-vs-hold threshold; the rename delay; the tap pair that opens an icon under a finger or pen (ms from press to press, CSS px from point to point); the drag slop — CSS px of travel before an icon's drag begins, for the mouse and for a finger or pen — and `dragSlop(event)`, which picks between them by `pointerType` |
