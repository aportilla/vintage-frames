#!/usr/bin/env python3
"""Cut the demo's desk-accessory icons out of the reference sheet.
...
def cut(pixels, box):
    """The bbox crop with edge-connected white flooded to transparent."""
    x0, y0, x1, y1 = box
    w, h = x1 - x0 + 1, y1 - y0 + 1
    ...
    stack = [
        ...
        for y in range(h)
        for x in range(w)
        if (x in (0, w - 1) or y in (0, h - 1)) and is_bg_white(crop[y][x])
    ]
    for x, y in stack:
        bg[y][x] = True
    while stack:
