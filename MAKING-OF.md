...
- **Scrollbars** — System 7's reserved-rail scrollbars, twice. First as a themed `::-webkit-scrollbar` skin, with the 1px frame painted *over* the scroller rather than as its border (WebKit pins native scrollbar rects to whole CSS pixels, so a border put the rail's anchors on a half CSS pixel at dpr 2 and set the whole rail one device pixel adrift in Safari). Then the retirement of that whole path (SCROLL-RAILS-PLAN.md): the engine owns themed-scrollbar geometry and quantizes it — measurement found three distinct WebKit regimes for one 48-device-px bar — so the kit now draws the rails itself as ordinary shadow DOM synced to the untouched native scrolling: the defect class became unrepresentable, the trough joined the exact-tile raster, Firefox got the full skin, the classic behaviors (fixed 16px thumb, trough paging, arrow auto-repeat) became first-class, and the rail became pixel-assertable headless for the first time.
...
- **The tile that walked off the grid** — the desktop dither's three-act fight with the layout grid (TILE-GRID-PLAN.md, ZOOM-TILE-DRIFT.md). Act one: a CSS repeating fill compounds one stored length's quantization error per repeat, fixed by spanning `lcm(motif, 15)` — until Safari's zoom mints scales with prime denominators no span covers. Act two: the placed-tile plan, whose geometry was perfect and whose paint wasn't — measurement showed Chromium rasterizes SVG images at the box's fractional layout size (ignoring `image-rendering` for SVG entirely) and antialiases every image background's unsnapped edge, so per-tile fills blend a hairline at every interior seam. Act three: one whole-surface raster at one image pixel per system pixel under `pixelated` — no seams to blend, nearest-neighbor can only emit source colors — with the placed grid kept as the consumer-token path. A story about how far "measure, don't model" had to be pushed before the model finally matched the paint.
