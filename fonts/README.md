...
glyph: the authored bitmap beside the embedded face's own rasterization at
12× (drift between them is a bug), context between strike neighbors, and
native-size copy — grouped by provenance, the *drawn* group first because it
is the one that wants design feedback. Its data file
